      ...
      for (var C = 0; C < F.length; C++) {
        ...
        if (E < 0) {
          var E = G.toLowerCase().indexOf(
            F[C].translate(
              "Ã¡Ã©Ã­Ã³ÃºÃ¼ÃÃ‰ÃÃ“ÃšÃœÃ Ã¨Ã¬Ã²Ã¹Ã€ÃˆÃŒÃ’Ã™",
...
usig.Callejero = (function (h) {
  ...
  function q(s) {
    ...
    if (s.length != usig.data.Callejero.length) {
      alert(
        "Se produjo un error al cargar la informaciÃ³n de cruces de calles."
      );
      return;
    }
  ...
  function j() {
    ...
    h.ajax({
      ...
      error: function () {
        alert(
          "Se produjo un error al intentar cargar la informaciÃ³n de calles."
        );
      },
    });
  }
...
usig.ErrorEnCargaDelCallejero = function () {
  ...
  this.getErrorMessage = function () {
    return "El callejero no se encuentra cargado aÃºn o se produjo un error al intentar cargarlo";
  };
};
if (typeof usig == "undefined") {
  usig = {};
    ...
    init: function (c) {
      ...
      for (var w = 1; w <= h.maxPalabras; w++) {
        re.calleAltura[w] = new RegExp(
          "(\\d+(\\s+(\\w|\\d|Ã¡|Ã©|Ã­|Ã³|Ãº|Ã¼|Ã±|'|`|,|\\.)+){" + w + "})",
          "gi"
        );
        re.calle[w] = new RegExp(
          "(\\w|\\d|Ã¡|Ã©|Ã­|Ã³|Ãº|Ã¼|Ã±|'|`|,|\\.)+(\\s+(\\w|\\d|Ã¡|Ã©|Ã­|Ã³|Ãº|Ã¼|Ã±|'|`|,|\\.)+){" +
            (w - 1) +
            "}",
          "gi"
        );
